...
- **应用图标**: 支持 Windows (.ico) 和 macOS (.icns) 原生图标
...
- **服务器端**: Linux (需要 uinput 支持,推荐 i.MX8MP 等嵌入式设备)
...
│   └── output/                   # 构建输出目录
│       └── remote_mouse_gui.app/ # macOS 应用程序包
...
1. 替换 `icons/logo.png` (基础图标)
2. 替换 `icons/logo.ico` (Windows 图标,需包含多种尺寸)
3. 替换 `icons/logo.icns` (macOS 图标)
4. 重新运行 `qmake` 和 `make`
